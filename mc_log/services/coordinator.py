...
class Coordinator:
    ...
    async def write_metrics(self, data: dict):
        ...
        if not data:
            return
        try:
            path = Path(get_astrbot_data_path()) / str(
                self._cfg().get("metrics_path", "audit_metrics.jsonl")
            )
            line = self.privacy_service.sanitize_for_persistence(
    ...
    async def extract_content(
        ...
        self.runtime.ensure_not_timed_out(
            deadline, run_id=run_id, stage="extract_start"
        )
        logger.info(
            f"[mc_log][{run_id}] 开始提取内容: file={local_file.name}, is_archive={is_archive}"
        )
        if is_archive:
            return await self.extract_from_archive(
                local_file, work_dir, run_id=run_id, deadline=deadline
            )
        content, strategy = await self.extract_selected_path(
            local_file, run_id=run_id, deadline=deadline
        )
        return ExtractionResult(
            content=content, source_name=local_file.name, strategy=strategy
        )
    ...
    async def extract_from_archive(
        ...
        self.runtime.ensure_not_timed_out(
            deadline, run_id=run_id, stage="extract_archive_start"
        )
        ext = archive_path.suffix.lower()
        extracted_paths: list[Path] = []
        extract_root = work_dir / ("unzipped" if ext == ".zip" else "ungz")
        logger.info(
            f"[mc_log][{run_id}] 开始解压归档文件: file={archive_path.name}, ext={ext}"
        )
        if ext == ".zip":
            extracted_paths = await self.file_adapter.safe_extract_zip(
                archive_path, extract_root, deadline=deadline
            )
        elif ext == ".gz":
            out_path = await self.file_adapter.safe_extract_gz(
                archive_path, extract_root, deadline=deadline
            )
        ...
        logger.info(
            f"[mc_log][{run_id}] 解压完成: extracted_count={len(extracted_paths)}"
        )
        preview_files = [
            f"{path.name}({path.stat().st_size}B)"
            for path in extracted_paths[:80]
            if path.exists()
        ]
        ...
        content, strategy = await self.extract_selected_path(
            selected, run_id=run_id, deadline=deadline
        )
        return ExtractionResult(
            content=content,
    ...
    async def handle_message(self, event):
        ...
        async def release_slot_once(reason: str):
            ...
            if slot_released:
                logger.info(
                    f"[mc_log][{run_id}] 跳过重复释放全局并发槽位: reason={reason}"
                )
                return
            slot_released = True
        ...
        async def watchdog_release():
            try:
                ...
                logger.warning(
                    f"[mc_log][{run_id}] 任务疑似卡住，watchdog 触发释放槽位"
                )
            ...
            try:
                ...
                if not self.prompt_manager.prompts_ready:
                    ...
                    if not analyze_provider_id:
                        logger.error(
                            f"[mc_log][{run_id}] 未配置模型提供商: analyze_select_provider={analyze_provider_id!r}"
                        )
                        result = event.plain_result(
                            self._msg("provider_not_configured")
                        )
                        result.stop_event()
                        terminal_result = result
                    elif not self.context.get_provider_by_id(analyze_provider_id):
                        logger.error(
                            f"[mc_log][{run_id}] 配置的模型提供商不存在: analyze={analyze_provider_id}"
                        )
                        result = event.plain_result(
                            self._msg("provider_not_configured")
                        )
                        ...
                        logger.info(
                            f"[mc_log] 文件命中规则: name={detect_file_name(event, file_comp)}, is_archive={is_archive}"
                        )
                        work_dir = self.runtime.build_work_dir(event)
                        run_ctx.work_dir = work_dir
                        work_dir.mkdir(parents=True, exist_ok=True)
                        (work_dir / ".mc_log_analysis").write_text(
                            "1", encoding="utf-8"
                        )

                        self.runtime.ensure_not_timed_out(
                            deadline, run_id=run_id, stage="before_download"
                        )
                        t_download = time.monotonic()
                        local_file = await self.file_adapter.download_to_workdir(
                            file_comp, work_dir, deadline=deadline
                        )
                        logger.info(
                        ...
                        if not local_file or not local_file.exists():
                            logger.warning(
                                f"[mc_log][{run_id}] 下载阶段失败，未获取到本地文件"
                            )
                            ...
                            if not extraction_result.content.strip():
                                logger.warning(
                                    f"[mc_log][{run_id}] 提取结果为空，无法继续分析"
                                )
                                result = event.plain_result(
                                    self._msg("no_extractable_content")
                                )
                                result.stop_event()
                                terminal_result = result
                            else:
                                extracted = self.extraction_domain.apply_total_budget(
                                    extraction_result.content, cfg["total_char_limit"]
                                )
                                extracted_for_llm = self.privacy_service.guard_for_llm(
                                    extracted
                                )
                                self.runtime.set_active_archive_file_map(
                                    event, extraction_result.archive_file_map
                                )
                                self.runtime.set_active_primary_source_name(
                                    event, extraction_result.source_name
                                )
                                available_archive_files = sorted(
                                    extraction_result.archive_file_map.keys()
                                )
                                t_analyze = time.monotonic()
                                report_md = (
                                    await self.analysis_service.analyze_with_llm(
                                        event=event,
                                ...
                                logger.info(
                                    f"[mc_log][{run_id}] 阶段分析: {time.monotonic() - t_analyze:.2f}s, ok={bool(report_md)}"
                                )
                                if not report_md:
                                    logger.warning(
                                        f"[mc_log][{run_id}] 最终分析未返回内容"
                                    )
                                    metrics_data["needs_more_info"] = True
                                    result = event.plain_result(
                                        self._msg("analyze_failed_logged")
                                    )
                                    result.stop_event()
                                    terminal_result = result
                                else:
                                    report_md = self.privacy_service.guard_for_output(
                                        report_md
                                    )
                                    metrics_data.update(
                                        self.metrics_service.extract_metrics_from_report(
                                            report_md
                                        )
                                    )
                                    ...
                                    if should_force_plain_text_output(event):
                                        logger.info(
                                            f"[mc_log][{run_id}] 平台强制纯文本输出: platform={event.get_platform_name()}"
                                        )
                                        ...
                                        (
                                            render_mode,
                                            render_payload,
                                        ) = await self.rendering_adapter.render_report(
                                            analysis_result.report_md,
                                            run_id=run_id,
                                            deadline=deadline,
                                        )
                                        ...
                                        logger.info(
                                            f"[mc_log][{run_id}] 阶段渲染: {time.monotonic() - t_render:.2f}s, "
                                    ...
                                    if (
                                        not should_force_plain_text_output(event)
                                        and analysis_result.code_blocks_text
                                    ):
                                        code_result = event.plain_result(
                                            analysis_result.code_blocks_text
                                        )
                ...
                await self.runtime.sync_latest_debug_copy(
                    lambda src, dst: self.file_adapter.copy_path(src, dst)
                )
        finally:
            if watchdog_task:
