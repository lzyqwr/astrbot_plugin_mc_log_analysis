...
- 内置工具：
  ...
  - `check_mod_fix_status`（查询嫌疑模组兼容新版 changelog，判断问题是否可能已被修复）
- 支持三种渲染模式：
  - `html_to_image`（模板渲染）
...
- 更新日志核验不会因为“存在新版”或泛泛的 `bug fixes` 就建议更新；只有 changelog 明确关联相同模块、崩溃类型或兼容问题时才会提高修复置信度。
...
- 压缩包：扩展名为 `.zip` 或 `.gz`，且文件名包含 `错误报告` / `crash` / `日志` / `log`
...
3. 如需限制可用范围，可选填写 `session_whitelist`。
4. 发送符合规则的日志文件到机器人会话。
...
| `messages.provider_not_configured` | `string` | `请先在插件配置中填写 analyze_select_provider。` | Provider 未配置提示 |
...
- 提示模板缺失
  - 检查 `assets` 下 2 个分析 prompt 文件是否存在且非空。
